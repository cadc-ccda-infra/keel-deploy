...
`kueue/localQueues/prod.src.yml` defines `LocalQueue` `src-default` in `canfar-src-workloads`, bound to cluster queue `src` (cohort `canfar`). Synced by Argo app `canfar-kueue-staging-src`, not by a Helm chart in this tree.
